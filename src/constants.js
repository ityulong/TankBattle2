...
export function getBattleThemeName(worldIndex) {
  if (!Number.isFinite(worldIndex)) return BATTLE_THEMES[0];
  const normalized = Math.abs(Math.trunc(worldIndex));
  return BATTLE_THEMES[normalized % BATTLE_THEMES.length];
}
